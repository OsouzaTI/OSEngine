...
void GraphicPipeline::read_obj_file(std::string name, const char* filename)
{
	Mesh temp_mesh;
	temp_mesh.load_obj_file_dataV2(filename);
	temp_mesh.set_display(display);		
	meshmap[name] = meshs.size();
	meshs.push_back(temp_mesh);

}

void GraphicPipeline::load_texture(const char* name, const char* filepath)
{
	get_mesh(name)->texture.load_png_image(filepath);
	get_mesh(name)->draw_mode = TEXTURED;
}
...
void GraphicPipeline::process_image()
{
	...
	for (auto& meshObj : meshs)
	{
		/* reset the number_of_triangles */
		meshObj.number_of_triangles_to_render = 0;

		vect3<float> target{ 0, 0, 30 };
		...
		for (int i = 0; i < n_faces; i++)
		{
			...
			mesh_face.color = meshObj.get_color();
			...
			for (int j = 0; j < 3; j++)
			{
				...
				world_matrix = Mat4x4_MultiplyMatrix(meshObj.shearXY_matrix, world_matrix);
				world_matrix = Mat4x4_MultiplyMatrix(meshObj.shearYX_matrix, world_matrix);
			...
			*/
			...
			/* Sombreamento*/
			if (meshObj.shading_type == SHADING_TYPE::FLAT) {
			...
			for (int t = 0; t < num_triangles_clipped; t++)
			{
				...
				if (meshObj.number_of_triangles_to_render < MAX_TRIANGLES_PER_MESH) {
					meshObj.triangles_to_render[meshObj.number_of_triangles_to_render] = projected_triangle;
					meshObj.number_of_triangles_to_render++;
				}
				else { break; }
			}
...
void GraphicPipeline::process_image2()
{
	/* this function calcule all matrix multiplications, backface culling, flat shading
	 ...
	 /* reset the number_of_triangles */
	meshs[0].number_of_triangles_to_render = 0;
	...
	for (int i = 0; i < n_faces; i++)
	{
		...
		face  mesh_face = mesh.faces[i];
		/* setando a cor selecionada pelo imgui */
		mesh_face.color = meshs[0].get_color();
		/* vetor responsavel por armazenar os vertices da face */
		vect3<float> face_vertex[3];
		...
		for (int j = 0; j < 3; j++)
		{
			...
			world_matrix = Mat4x4_MultiplyMatrix(rotateX, world_matrix);
			world_matrix = Mat4x4_MultiplyMatrix(rotateZ, world_matrix);
			world_matrix = Mat4x4_MultiplyMatrix(meshs[0].shearXY_matrix, world_matrix);
			world_matrix = Mat4x4_MultiplyMatrix(meshs[0].shearYX_matrix, world_matrix);
			world_matrix = Mat4x4_MultiplyMatrix(translation, world_matrix);
			transformed_vertex = Mat4x4_MultiplyVector(world_matrix, transformed_vertex);
		...
		*/
		...
		for (int t = 0; t < num_triangles_clipped; t++)
		{
			...
			if (meshs[0].number_of_triangles_to_render < MAX_TRIANGLES_PER_MESH) {
				meshs[0].triangles_to_render[meshs[0].number_of_triangles_to_render] = projected_triangle;
				meshs[0].number_of_triangles_to_render++;
			}
			else { break; }
		}
...
void GraphicPipeline::draw_pipeline()
{
	for (auto& meshObj : meshs)
	{
		uint32_t color = meshObj.get_color();
		int number_of_triangles_to_render = meshObj.number_of_triangles_to_render;
		triangle* triangles_to_render = meshObj.triangles_to_render;
		OSTexture texture = meshObj.texture;
		...
		for (int i = 0; i < n_triangles; i++)
		{
			...
			if (meshObj.draw_mode == DRAW_MODE::WIREFRAME) {
				draw->draw_triangle(
					triangle->points[0].x,
					triangle->points[0].y,
					triangle->points[1].x,
			...
			if (meshObj.draw_mode == DRAW_MODE::FILLED) {
			...
			if (meshObj.draw_mode == DRAW_MODE::TEXTURED) {
			...
			if (meshObj.draw_mode == DRAW_MODE::TEXTURED_WIREFRAME) {
				// texture
...
Mesh* GraphicPipeline::get_mesh(const char* name)
{
	for (auto& mesh_index : meshmap)
	{
		if (mesh_index.first.compare(name) == 0) {			
			return &meshs[mesh_index.second];
		}
	}
	return NULL;
}
